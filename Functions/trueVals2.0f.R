...
#'
#' @export
trueVals2.0f <- function(data_list, 
        ...
        if (Mfamily == "binomial") {
            ...
            if (Yfamily == "gaussian") {
                ...
                M_draws <- sapply(1:num_draws, function(k) {
                    rnorm(N, mean = m_latent, sd = sqrt(1 - gen_m$iccm)) #test_list$parameters$iccm))
                })
                ...
                E_y <- rowMeans(
                    pnorm(y_latent_mat, mean = 0, sd = sqrt(1 - iccy))
                )
