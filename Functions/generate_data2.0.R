...
#' @examples
#' # Example usage
#' result <- generate_data2.0(J = 50, num_x = 2, Mfamily="gaussian", Yfamily="gaussian")
#' head(result$data)
#' 
#' @export
generate_data2.0 <- function(J = 100,                        # Number of clusters
    ...
    if (Mfamily %in% c("binomial","gaussian") & Yfamily %in% c("binomial","gaussian")) {
        true_vals <- trueVals2.0(data_list = data_list)
    } else {
        true_vals <- NULL
    }
